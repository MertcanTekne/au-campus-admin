...
import { AdminSection, PanelRole, PanelUser, UserRole } from "@/types/admin";

type AuthStatus = "checking" | "signed-out" | "checking-role" | "allowed" | "blocked";
...
function getBlockedMessage(role: string | null) {
  if (role === "student") {
    return "Öğrenci hesapları web paneli kullanamaz. Lütfen AÜ Campus mobil uygulamasını kullanın.";
  }

  if (role === "academic_pending") {
    return "Akademik hesabınız henüz admin onayı bekliyor.";
  }

  return "Bu panele erişim yetkiniz yok.";
}

function isPanelRole(role: string | null): role is PanelRole {
...
export default function Home() {
  ...
  const [blockedRole, setBlockedRole] = useState<UserRole | string | null>(null);
  ...
  async function checkPanelAccess(user: User) {
    ...
    try {
      ...
      const role = userSnapshot.exists() ? String(userSnapshot.data().role || "") : null;

      if (isPanelRole(role)) {
        setPanelUser({ uid: user.uid, email: user.email || "", role });
      ...
      setBlockedRole(role);
      setStatus("blocked");
      setErrorMessage(getBlockedMessage(role));
    } catch {
      setBlockedRole(null);
      setStatus("blocked");
  ...
  async function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await checkPanelAccess(credential.user);
    } catch (error) {
      setStatus("signed-out");
  ...
  if (status === "allowed" && panelUser) {
    const visibleActiveSection =
      panelUser.role === "academic" && (activeSection === "users" || activeSection === "approvals")
        ? "classes"
        : activeSection;
    ...
    return (
      <AdminLayout
        ...
        panelRole={panelUser.role}
        userEmail={panelUser.email}
        isBusy={isBusy}
        onLogout={handleLogout}
        onSectionChange={(section) => {
          if (panelUser.role === "academic" && (section === "users" || section === "approvals")) {
      ...
      >
        {visibleActiveSection === "dashboard" ? <DashboardOverview panelUser={panelUser} /> : null}
        {panelUser.role === "admin" && visibleActiveSection === "users" ? (
          <UsersPanel currentUid={panelUser.uid} />
        ) : null}
        {panelUser.role === "admin" && visibleActiveSection === "approvals" ? (
          <AcademicApprovalsPanel />
        ) : null}
        {visibleActiveSection === "classes" ? <ClassesPanel panelUser={panelUser} /> : null}
      </AdminLayout>
    );
  }

  const panelTitle =
    panelUser?.role === "academic" || blockedRole === "academic"
      ? "Akademik Panel"
      : "Admin Panel";
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-5 py-8 sm:px-8 lg:px-10">
        ...
        <section className="flex flex-1 items-center justify-center py-10">
          ...
          {status === "signed-out" ? (
            ...
            >
              <div className="mb-7">
                <p className="text-sm font-medium text-[#e15b6a]">Panel girişi</p>
                <h2 className="mt-2 text-3xl font-semibold tracking-tight text-white">
                  AÜ Campus Yönetim
                </h2>
                <p className="mt-3 text-sm leading-6 text-zinc-400">
                  Admin veya akademik hesabınızla giriş yapın.
                </p>
              </div>
          ...
          {status === "blocked" ? (
            <div className="w-full max-w-lg rounded-lg border border-red-500/25 bg-red-500/10 p-6 text-center">
              ...
              <p className="mt-3 text-sm leading-6 text-red-100">
                {errorMessage || getBlockedMessage(blockedRole)}
              </p>
              <button
                type="button"
                onClick={handleLogout}
