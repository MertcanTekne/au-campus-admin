...
import {
  fetchClassesForPanel,
  formatDate,
  getFirestoreErrorMessage,
  getFirestoreWriteErrorMessage,
  updateClassActiveState,
} from "@/lib/adminData";
import { CampusClass, PanelUser } from "@/types/admin";
...
export function ClassesPanel({ panelUser }: ClassesPanelProps) {
  ...
  async function loadClasses() {
    ...
    setErrorMessage("");
    try {
      const classList = await fetchClassesForPanel(panelUser);
      setClasses(classList);
      setSelectedClass((currentSelected) =>
        currentSelected
          ? classList.find((campusClass) => campusClass.id === currentSelected.id) || null
          : null,
      );
    } catch {
  ...
  const filteredClasses = useMemo(() => {
    ...
    return classes.filter((campusClass) => {
      const searchable = [
        campusClass.className,
        campusClass.courseCode,
        campusClass.instructorEmail,
      ]
        .filter(Boolean)
        .join(" ")
      ...
      const activeMatches =
        activeFilter === "all" ||
        (activeFilter === "active" && campusClass.isActive === true) ||
        (activeFilter === "inactive" && campusClass.isActive === false);
  ...
  function applyUpdatedClass(updatedClass: CampusClass) {
    setClasses((currentClasses) =>
      currentClasses.map((campusClass) =>
        campusClass.id === updatedClass.id ? updatedClass : campusClass,
      ),
    );
  ...
  const title = panelUser.role === "admin" ? "Sınıflar" : "Sınıflarım";
  const description =
    panelUser.role === "admin"
      ? "Sınıfları görüntüleyin, oluşturun ve akademik içerikleri yönetin."
      : "Kendi sınıflarınızı, duyurularınızı ve materyallerinizi yönetin.";
    ...
    <section>
      <div className="mb-6 flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_180px_auto_auto]">
          <input
            ...
            placeholder="Ders adı, kod veya eğitmen ara"
          />
          <select
            value={activeFilter}
          ...
          <button
            ...
            className="h-10 rounded-md bg-[#b21f35] px-4 text-sm font-semibold text-white transition hover:bg-[#c62940]"
          >
            Yeni Sınıf Oluştur
          </button>
        </div>
      ...
      <div className="grid gap-5 2xl:grid-cols-[minmax(0,1fr)_560px]">
        <div className="overflow-hidden rounded-lg border border-white/10 bg-white/[0.03]">
          {isLoading ? (
            <div className="p-6 text-sm text-zinc-300">Sınıflar yükleniyor...</div>
          ) : filteredClasses.length === 0 ? (
            <div className="p-6 text-sm text-zinc-300">Sınıf bulunamadı.</div>
          ) : (
                  ...
                  {filteredClasses.map((campusClass) => (
                    <tr key={campusClass.id} className="align-top">
                      <td className="px-4 py-3 font-medium text-white">
                        {campusClass.className || campusClass.courseCode || "İsimsiz sınıf"}
                      </td>
                      ...
                      <td className="px-4 py-3 break-all text-zinc-300">
                        {campusClass.instructorEmail || "-"}
                      </td>
                      <td className="px-4 py-3 font-mono text-xs text-zinc-300">
                        {campusClass.joinCode || "-"}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`rounded-md px-2 py-1 text-xs font-medium ${
                            campusClass.isActive
                              ? "bg-emerald-500/15 text-emerald-100"
                              : "bg-zinc-500/15 text-zinc-300"
                          }`}
                        >
                          {campusClass.isActive ? "Aktif" : "Pasif"}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-zinc-300">
                        {formatDate(campusClass.createdAt)}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          <button
                            type="button"
                            onClick={() => setSelectedClass(campusClass)}
                            className="rounded-md bg-[#b21f35] px-3 py-2 text-xs font-semibold text-white hover:bg-[#c62940]"
                          >
                            Aç
                          </button>
                          <button
                            type="button"
                            onClick={() => handleToggleActive(campusClass)}
                            disabled={updatingClassId === campusClass.id}
                            className="rounded-md border border-white/15 px-3 py-2 text-xs font-semibold text-zinc-100 hover:bg-white/5 disabled:opacity-60"
                          >
                            {campusClass.isActive ? "Pasif" : "Aktif"}
                          </button>
                        </div>
                      </td>
                    </tr>
        ...
        {selectedClass ? (
          <ClassDetailPanel
            ...
            onClassUpdated={applyUpdatedClass}
          />
        ) : (
          <div className="rounded-lg border border-dashed border-white/15 bg-white/[0.02] p-6 text-sm text-zinc-400">
            Detayları görmek için bir sınıf seçin.
          </div>
        )}
      </div>
