...
import { CampusClass, ClassDetail, PanelUser } from "@/types/admin";
...
export function ClassDetailPanel({
  ...
  const canManage = canManageClass(panelUser, selectedClass);
  ...
  }, [canManage, selectedClass.id]);

  if (!canManage) {
    return (
      <aside className="rounded-lg border border-red-500/25 bg-red-500/10 p-5">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-red-100">Yetkisiz sınıf</p>
            <h3 className="mt-2 text-2xl font-semibold text-white">
              {selectedClass.className || selectedClass.courseCode || "Sınıf"}
            </h3>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-md border border-white/15 px-3 py-2 text-sm text-zinc-100 hover:bg-white/5"
          >
            Kapat
          </button>
        </div>
        <p className="text-sm leading-6 text-red-100">Bu sınıfı yönetme yetkiniz yok.</p>
      </aside>
    );
  }
  ...
  async function handleAnnouncementSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await addAnnouncement(selectedClass.id, {
        ...
        authorUid: panelUser.uid,
        authorEmail: panelUser.email,
      });
  ...
  return (
    <aside className="rounded-lg border border-white/10 bg-zinc-950 p-5">
      ...
      <dl className="grid gap-3 text-sm sm:grid-cols-2">
        <div>
          <dt className="text-zinc-500">Ders kodu</dt>
          <dd className="mt-1 text-zinc-100">{selectedClass.courseCode || "-"}</dd>
        </div>
        <div>
          <dt className="text-zinc-500">Şube</dt>
          <dd className="mt-1 text-zinc-100">{selectedClass.section || "-"}</dd>
        </div>
        <div>
          <dt className="text-zinc-500">Dönem</dt>
          <dd className="mt-1 text-zinc-100">{selectedClass.term || "-"}</dd>
        </div>
        <div>
          <dt className="text-zinc-500">Eğitmen</dt>
          <dd className="mt-1 break-all text-zinc-100">{selectedClass.instructorEmail || "-"}</dd>
        </div>
        <div>
          ...
          <dd className="mt-1 flex items-center gap-2">
            ...
            <button
              ...
              className="rounded-md border border-white/15 px-2 py-1 text-xs text-zinc-100 hover:bg-white/5"
            >
              Kopyala
            </button>
          </dd>
        ...
        <div>
          <dt className="text-zinc-500">Katılımcı sayısı</dt>
          <dd className="mt-1 text-zinc-100">
            {isLoading ? "Yükleniyor..." : detail?.memberCount ?? 0}
          </dd>
        </div>
      </dl>
      ...
      {canManage ? (
        <button
          ...
          className="mt-5 h-10 rounded-md bg-[#b21f35] px-4 text-sm font-semibold text-white transition hover:bg-[#c62940] disabled:opacity-60"
        >
        ...
        <div className="mt-6 grid gap-5 xl:grid-cols-2">
          <section>
            ...
            <form
              ...
              className="mt-3 rounded-md border border-white/10 bg-white/[0.03] p-4"
            >
              <input
                value={announcementTitle}
                onChange={(event) => setAnnouncementTitle(event.target.value)}
                placeholder="Başlık"
                className="h-10 w-full rounded-md border border-white/10 bg-zinc-900 px-3 text-sm text-zinc-50 outline-none placeholder:text-zinc-500 focus:border-[#b21f35]"
              />
              <textarea
                ...
                placeholder="İçerik"
                rows={3}
                className="mt-3 w-full rounded-md border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-zinc-50 outline-none placeholder:text-zinc-500 focus:border-[#b21f35]"
              />
              <button
                type="submit"
                disabled={isSubmitting}
                className="mt-3 h-9 rounded-md bg-[#b21f35] px-3 text-sm font-semibold text-white disabled:opacity-60"
              >
                Duyuru Ekle
              </button>
            </form>
            <div className="mt-3 grid gap-3">
              {isLoading ? (
                <p className="rounded-md border border-white/10 bg-white/[0.03] p-4 text-sm text-zinc-300">
                ...
                detail.announcements.map((announcement) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <p className="font-medium text-white">{announcement.title || "Duyuru"}</p>
                      <button
                        type="button"
          ...
          <section>
            ...
            >
              ...
              <select
                ...
                className="mt-3 h-10 w-full rounded-md border border-white/10 bg-zinc-900 px-3 text-sm text-zinc-50 outline-none focus:border-[#b21f35]"
              >
              ...
              <input
                ...
                placeholder="URL"
                className="mt-3 h-10 w-full rounded-md border border-white/10 bg-zinc-900 px-3 text-sm text-zinc-50 outline-none placeholder:text-zinc-500 focus:border-[#b21f35]"
              />
              <button
                type="submit"
                disabled={isSubmitting}
            ...
            <div className="mt-3 grid gap-3">
              {isLoading ? (
                ...
                detail.materials.map((material) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <p className="font-medium text-white">{material.title || "Materyal"}</p>
                      <button
                        type="button"
                    ...
                    {material.content ? (
                      <p className="mt-2 text-sm leading-6 text-zinc-300">{material.content}</p>
                    ) : null}
                    {material.url ? (
                      <p className="mt-2 break-all text-sm text-[#ffb3bd]">{material.url}</p>
                    ) : null}
                    <p className="mt-3 text-xs text-zinc-500">
