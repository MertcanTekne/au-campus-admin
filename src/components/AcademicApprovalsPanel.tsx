...
export function AcademicApprovalsPanel() {
  ...
  async function handleDecision(uid: string, role: Extract<UserRole, "academic" | "student">) {
    ...
    try {
      ...
      setSuccessMessage(
        role === "academic"
          ? "Kullanıcı akademik olarak onaylandı."
          : "Kullanıcı öğrenci rolüne çevrildi.",
      );
    } catch {
    ...
    <section>
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-zinc-400">
            academic_pending rolündeki kullanıcıları değerlendirin.
          </p>
        </div>
        <button
          type="button"
      ...
      {isLoading ? (
        <div className="rounded-lg border border-white/10 bg-white/[0.04] p-6 text-sm text-zinc-300">
          Onay listesi yükleniyor...
        </div>
      ) : users.length === 0 ? (
        <div className="rounded-lg border border-white/10 bg-white/[0.04] p-6 text-sm text-zinc-300">
          Onay bekleyen akademik kullanıcı yok.
        </div>
      ) : (
          ...
          {users.map((user) => (
            <article key={user.uid} className="rounded-lg border border-white/10 bg-white/[0.04] p-5">
              <p className="text-lg font-semibold text-white">
                {user.displayName || user.name || "İsim bilgisi yok"}
              </p>
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                <button
                  type="button"
                  onClick={() => handleDecision(user.uid, "academic")}
                ...
                >
                  Akademik olarak onayla
                </button>
                <button
                  type="button"
                  onClick={() => handleDecision(user.uid, "student")}
                  disabled={updatingUid === user.uid}
                  className="h-10 rounded-md border border-white/15 px-4 text-sm font-semibold text-zinc-100 transition hover:bg-white/5 disabled:opacity-60"
                >
                  Öğrenciye çevir
                </button>
              </div>
